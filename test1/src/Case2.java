...
//					if(link.attr("href").contains(num1+"") && !link.text().equals("") ){
			if( !( link.text().equals( "複製連結" ) ) && !( link.text().equals( "" ) ) )
			{
				aa.append( link.text() );
				System.out.println( link.text() );
				aa.append( "\n" );
...
//					}
	...
	public static StringBuffer name()
	{
		...
		try
		{
			FileReader fr = new FileReader( "d:\\PostToGetTitleAndLink\\txt\\case2.txt" );
			BufferedReader br = new BufferedReader( fr );
//			<div class="text-center">
			try
			{
				while( !( strNum = StringUtils.trimToEmpty( br.readLine() ) ).equals( "<div class=\"text-center\">" ) )
				{
					if( strNum.equals( "<div class=\"list-item clearfix\">" ) )
					{
						while( !( strNum = StringUtils.trimToEmpty( br.readLine() ) ).equals( "<div class=\"user-info\">" ) )
						{
